...
define Build/tenbay-factory
  ...
  mv "$@" "$@.tmp/UploadBrush-bin.img"
  $(MKHASH) md5 "$@.tmp/UploadBrush-bin.img" | head -c32 > "$@.tmp/check_MD5.txt"
  echo -n $$(cat "$@.tmp/check_MD5.txt" | head -c32)$$(echo -n $$(echo $(1) | cut -d' ' -f2)$$(echo $(1) | cut -d' ' -f1) | $(MKHASH) md5 | head -c32) | $(MKHASH) md5 | head -c32 >"$@.tmp/bin_random_oem.txt"
